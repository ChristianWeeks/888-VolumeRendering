...
std::vector<float> rayMarch(Camera cam, lux::Vector n, float start, float end){
    ...
    //    C[0] = 1.0;
    //}
    std::vector<float> smokeColor {1.0, 1.0, 1.0, 1.0};
...
void initVolumes(){
    ...
    //Implicit functions
    auto sphere = std::make_shared<lux::SphereVolume<float> >(1.3);
    ...
    auto box = std::make_shared<lux::BoxVolume<float> >(0.9, 6.0);
    ...
    auto pyroSphere = std::make_shared<lux::PyroSphereVolume>(3, 2.0);
/*
...
int main(int argc, char **argv){
    ...
    Camera mainCam;
    mainCam.setEyeViewUp(lux::Vector(0.0, 0.0, 20), lux::Vector(0,0,-1), lux::Vector(0,1,0));
    float bbSize = 4.0;
    ...
    for(int i = 0; i < w; i++){
        for(int j = 0; j < h; j++){
           // std::cout << "3\n";
            ...
            lux::Vector r = mainCam.view(normalizedPixelCoordX, normalizedPixelCoordY);
            Ray ray(mainCam.eye(), r);
            std::vector<float> intersects = bb.intersect(ray, 0, 50);
            if(intersects[0] != -1.0){
                std::vector<float> C = rayMarch(mainCam, r, intersects[0], intersects[1]);
                img.setPixel( i, j, C);
            }
            else{
    ...
    //std::string elapsedTime(t.elapsed());
